...
const hookAddPocButtonDirectly = () => {
  ...
  /* =======================================================
   ...
   - Additional Trainers “+” button now works reliably in ALL markup cases
     (even if the button moved, lost attributes, or is inside/outside the card)
   - Additional POC “+” button now works reliably in ALL markup cases
   - We expose addTrainerRow / addPocCard + the button detectors safely on window
   - ✅ FIXED a break: startExpandObserver() was missing / observer block was malformed
...
(() => {
  ...
  // ✅ expose for safety-net + external callers
  try {
  ...
  // ✅ FIXED: startExpandObserver was missing / malformed in your pasted version
  let __mkExpandObserverStarted = false;
  const startExpandObserver = () => {
    if (__mkExpandObserverStarted) return;
    __mkExpandObserverStarted = true;
  ...
  document.addEventListener("click", (e) => {
    ...
    if (navBtn) {
      ...
      setTimeout(() => {
        ...
        // re-hook add-trainer + add-poc button when page becomes active
    ...
    if (resetBtn) {
      e.preventDefault();
      const section =
        (resetBtn.dataset.clearPage && document.getElementById(resetBtn.dataset.clearPage)) || resetBtn.closest(".page-section");
  ...
  const init = () => {
    ...
    // ✅ bind trainer + POC button directly too (in case of weird overlays/stopPropagation elsewhere)
    hookAddTrainerButtonDirectly();
    hookAddPocButtonDirectly();
